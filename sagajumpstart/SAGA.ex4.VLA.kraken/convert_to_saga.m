...
for i = 1:nf
 index = (i-1)*nels+1;
 % under UNIX
 %filein = ['kraken/csdm.rep' int2str(f(i))]
 %a(1:nels,index:index+nels-1)=conj(loadsio(filein,'complex'));
 % under Linux
 filein = ['kraken/csdm.rep.',int2str(f(i)),'Hz.sio']
 d=sioread(filein,1,0,[1:nels*2]);                 % read in data file
 c=conj(complex(d(:,[1:2:nels*2-1]),d(:,[2:2:nels*2]))); % put data in complex format
% c=complex(d(:,[1:2:nels*2-1]),d(:,[2:2:nels*2])); % put data in complex format
